...
class PhpTagParser implements IMacroParser
{
    ...
    public function parse(string $content): string
    {
        ...
        $resultContent = preg_replace('/<\?php([\s]*?)([\r\n]*)/', sprintf('%s echo \'%s\'; %s$1$2$2', self::PHP_TAG_BEGIN, self::PHP_TAG_BEGIN, self::PHP_TAG_END), $resultContent);
        $resultContent = preg_replace('/\?>/', sprintf('%s echo \'%s\'; %s', self::PHP_TAG_BEGIN, self::PHP_TAG_END, self::PHP_TAG_END), $resultContent);
